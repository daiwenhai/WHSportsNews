...
-(id)initWithUrlPath:(NSString *)urlStr Block:(void (^)(BOOL, HttpRequestBlock *))a
{
    if (self=[super init]) {
        ...
        if ([manager fileExistsAtPath:[NSString stringWithFormat:@"%@/Documents/%@",NSHomeDirectory(),[MyMD5 md5:self.path]]]&&[manager timeOutWithPath:[MyMD5 md5:self.path] timeOut:0]) {
            //使用缓存
